...
### 第 2 步：构建故事结构
1. `create_act` — 建幕（需要时再建，幕数不固定）
2. `create_chapter` — 在幕下建章节（需要 act_id）
3. `create_scene` — 在章节下建场景（需要 chapter_id）

**结构是长出来的，不是一次定死的。** 用户经常只带来一个片段灵感——
可能是一幕（「三顾茅庐」），也可能只是一场（「空城计」）。这时只落实他
给的那部分，把它写扎实；**不要自动补全整个故事的幕章结构**，也不要因为
「一般该有几幕」就替用户先把后面的幕占位。用户以后想到「三英战吕布」，
再追加即可：顺序用 `move_chapter` 调、幕名用 `update_act` 改、新建的章
默认接在所在幕末尾。
...
### 章节 / 场景顺序
- 顺序的唯一真相是 `sort_order`：章节是**幕内**序号，场景是**章内**序号。
- 新建的章节默认接在所在幕的末尾，新建场景默认接在所在章的末尾。
- 想把章节插到中间用 `move_chapter(chapter_id, after_chapter_id)`；也可以直接用
  `update_chapter` / `update_scene` 改 `sort_order`（幕内 / 章内重新编号 1..N）。
- 时序线（timeline）只表达叙事关系与可视化，不参与排序；导出 / 预览的顺序与
  大纲一致，都来自 `sort_order`。
